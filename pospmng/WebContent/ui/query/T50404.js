Ext.onReady(function() {
	...
	var txnGridStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url : 'gridPanelStoreAction.asp?storeId=sameMccAmt2'
		}),
				...
				tbar : [
						{
							...
							handler : function() {
								...
								if (txnGridStore.getTotalCount() < System[REPORT_MAX_COUNT]) {
									Ext.Ajax
											.request({
												url : 'T50404Action.asp',
												timeout : 60000,
												params : {
													queryStartTm :Ext.get('queryStartTm').getValue().replace('-','').replace('-',''),
													queryEndTm :Ext.get('queryEndTm').getValue().replace('-','').replace('-','')
												},
						...
						'-',
						{
							...
							handler : function() {
								...
								//console.log(Ext.get('queryEndTm').getValue().replace('-','').replace('-',''));
							//console.log(typeof(Ext.get('queryEndTm').getValue()));	
