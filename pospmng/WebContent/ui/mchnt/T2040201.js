...
//						hiddenName: 'conn_inst_cd',
		        	...
		        	xtype: 'panel',
		       		items: [{
							...
							'listeners':{
								'select': function(){
									var amMchtTp = mchntForm.getForm().findField('mcht_type').getValue();
									var realMchtTp = mchntForm.getForm().findField('real_mcht_tp').getValue();
        ...
        buttons: [{
            ...
            text: '返回',
            handler: function() {
            	window.location.href = Ext.contextPath + '/page/mchnt/T20402.jsp';
			}
        }]
    });
    ...
    Ext.getCmp("card_in_settle_bankI").on('select',function(){
    	T20401.getInfo(Ext.getCmp("card_in_settle_bankI").getValue(),function(ret){
    		if(ret=='0'){
    ...
    function subSave(){
		...
		if (frm.isValid()) {
			...
			frm.submitNeedAuthorise({
				url: 'T20402Action.asp?method=update',
				waitTitle : '请稍候',
