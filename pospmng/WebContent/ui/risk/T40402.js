Ext.onReady(function() {
	...
	function createWhiteMchtDtl(){
		...
		Ext.getCmp('mchtCaseDespF').setValue(rec.get('mchtCaseDespF'));
	}
	...
	var whiteMchtGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=whiteMchtLQuery'
		}),
		reader: new Ext.data.JsonReader({
			...
			{name: 'mchtCaseDespF',mapping: 'mchtCaseDespF'}
		]),
		autoLoad: true
	});
		...
		buttons : [{
			...
			handler : function() {
				whiteMchtForm.getForm().submit({
					url : 'T40402Action.asp?method='+method,
					waitMsg : '正在提交，请稍后......',
