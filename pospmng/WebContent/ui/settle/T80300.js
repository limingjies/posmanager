Ext.onReady(function() {
	...
	var backGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=tblNTxnHis'
		}),
...
//		{header: '清算日期',dataIndex: 'dateSettlmt8',hidden: true,renderer: formatDt},
		{header: '系统流水号',dataIndex: 'sysSeqNum',hidden: true},
...
//		plugins: rowExpander,
		...
		tbar: [{
			...
			disabled : true,
			handler:function() {
				showConfirm('确定要对此业务信息申请退货吗？', backGrid, function(bt) {
						// 如果点击了提示的确定按钮
						if (bt == "yes") {
							
							Ext.Ajax.request({
										url : 'T80300Action.asp?method=apply',
										
										params : {
											instDate: backGrid.getSelectionModel().getSelected().data.instDate,
											sysSeqNum: backGrid.getSelectionModel().getSelected().data.sysSeqNum,
											txnId : '80300',
											subTxnId : '01'
										},
	...
	backGrid.getSelectionModel().on({
		'rowselect': function() {
			...
			detailGridStore.load({
				params: {
					start: 0,
					dateSettlmt8: backGrid.getSelectionModel().getSelected().data.instDate,
					keyCup: backGrid.getSelectionModel().getSelected().data.sysSeqNum
				}
			})
