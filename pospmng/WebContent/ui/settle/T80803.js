Ext.onReady(function() {
	...
	var paySettleGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=paySettle'
		}),
	...
	var detailGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=paySettleDtl'
		}),
		reader: new Ext.data.JsonReader({
			root: 'data',
			totalProperty: 'totalCount'
			...
			handler:function() {
				showConfirm('确定要下载一下全部划付文件吗？', paySettleGrid, function(bt) {
						...
						if (bt == "yes") {
							...
							Ext.Ajax.request({
								url : 'T80803Action.asp?method=downloadAll',
								params : {
									instDate: stlDate,
									txnId : '80803',
									subTxnId : '01'
					...
					handler : function() {
						if (downloadForm.getForm().isValid()) {
							downloadForm.getForm().submit({
								url : 'T80803Action.asp?method=download',
								waitMsg : '正在提交，请稍后......',
