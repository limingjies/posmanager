...
@Override
public void updateByTremId(String mchtId, String newMchtNo,
		String currentDateTime) {
		String sql="update tbl_term_key set mcht_cd ='"+newMchtNo+"' ,rec_upd_ts='"+currentDateTime+"' where mcht_cd='"+mchtId+"'";
		this.getSession().createSQLQuery(sql).executeUpdate();
}
@Override
public List<TblTermKey> getByMchnt(String mchntId) {
	String hql="from com.allinfinance.po.TblTermKey t where t.Id.MchtCd ='"+mchntId+"'";
//	DetachedCriteria criteria=DetachedCriteria.forClass(TblTermKey.class);
//	criteria.add(Restrictions.eq("MchtCd", mchntId));
	Query query = this.getSession().createQuery(hql);
	List<TblTermKey> list = query.list();
	return list;
}
}