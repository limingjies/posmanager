...
public class T10501Action extends BaseAction {
	...
	protected String subExecute() throws Exception {
		try {
			...
			log("操作员编号：" + operator.getOprId() + "，机构密钥同步操作" + getMethod()
	...
	//
	// //String bitStr =
	// "3030373360000380094C5249001C000000000269080600000000026909055800094900060000003200016001000000000800800000000080000004000000000000003039393939393939313031";
	//
	// String sendMsgInfo = "81010800" + "09999999" + "101" ;
	//
	// // "报文内容长度" 4位 不足左补0
	// String sendMsgInfoLen =
	// CommonFunction.fillString(String.valueOf(sendMsgInfo.length()), '0', 4,
	// false);
	//
	// // "最终发送的报文" = "报文内容长度" + "报文内容"
	...
	//
	// keyByteHigh = (byte) (bitStr.charAt(j) > '9' ? bitStr.charAt(j) - 'A' +
	// 0x0a : bitStr.charAt(j) - '0');
	//
	// j++;
	//
	// keyByteLow = (byte) (bitStr.charAt(j) > '9' ? bitStr.charAt(j) - 'A' +
	// 0x0a : bitStr.charAt(j) - '0');
	//
	...
	private String syKey() throws Exception {
		...
		log(command[1].toString());
		log(String.valueOf(len));
		...
		log("机构密钥同步result:"+result);
		if (result.length() > 2) {
			log("机构密钥同步应答码result(8, 10):"+result.substring(8, 10));
...
//		String cmdStr = txnCode + cardNo + termNo + exFixed;
		...
		cmdStr.append("6101");
		//cmdStr.append(CommonFunction.fillString("0", '0', 27, true));
		//cmdStr.append("027");
		cmdStr.append("1234567890123456789");
		cmdStr.append(insIdCd.trim() + "aaaa");
		cmdStr.append("023");
		cmdStr.append(CommonFunction.fillString(StringUtil.isNotEmpty(firstMchtNo)?firstMchtNo.trim():"000000000000000", ' ', 15, true));
		cmdStr.append(CommonFunction.fillString(StringUtil.isNotEmpty(firstTermId)?firstTermId.trim():"00000000",' ',8,true));
		log(cmdStr.toString());
		byte[] cmdStrBytes = cmdStr.toString().getBytes();
		len = cmdStrBytes.length;
		String lenStr = CommonFunction.fillString(String.valueOf(len), '0', 4, false);
		byte[] lenBytes = lenStr.getBytes();
		
		byte[] cmd = new byte[cmdStrBytes.length+lenBytes.length];
		System.arraycopy(lenBytes, 0, cmd, 0, lenBytes.length);
		System.arraycopy(cmdStrBytes, 0, cmd, lenBytes.length, cmdStrBytes.length);
		log(new String(cmd));
		return new Object[]{cmd, CommonFunction.fillString(String.valueOf(cmdStrBytes.length+lenBytes.length), '0', 4, false)};
	}
