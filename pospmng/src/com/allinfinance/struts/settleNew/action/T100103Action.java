 ...
 * @version 1.0
 */
public class T100103Action extends BaseAction {
	
	private static final long serialVersionUID = 1L;
	
	
	T100103BO t100103BO = (T100103BO) ContextUtil.getBean("T100103BO");
	...
	public String start() throws UnsupportedEncodingException {
		...
		if (connectState == null) {
			...
			rspCode = "启动失败，无状态返回！";
		} else if (Constants.SUCCESS_CODE.equals(CommonFunction.mySubstr(connectState,1,3))){
			rspCode = Constants.SUCCESS_CODE;
		} else {
			rspCode = CommonFunction.mySubstr(connectState,3,256).trim();
		}
		return rspCode;
	}
	...
	public String rest() {
		try {
			rspCode = t100103BO.update(restFlag,restBatId,restGrpId);
		} catch (Exception e) {
			e.printStackTrace();
			rspCode = "重置批量失败！";
		}
	...
	public String getBatchMainPort() {
		return SysParamUtil.getParam(SysParamConstants.SOCKET_BATCH_PORT_NEW);
	}
