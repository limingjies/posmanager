...
import com.allinfinance.po.settleNew.TblSettleDtl;
 ...
 *
 */
public class T100304BOTarget implements T100304BO {
...
//	private static Logger log = Logger.getLogger(T80803BOTarget.class);
	...
	public String allFile(String instDate) throws Exception {
		...
		List<TblSettleDtl> dataListZX =getDataList(instDate,SysParamUtil.getParam(SysParamConstants.CHANNEL_CODE_ZX));
		List<TblSettleDtl> dataListGH =getDataList(instDate,SysParamUtil.getParam(SysParamConstants.CHANNEL_CODE_GH));
		List<TblSettleDtl> dataListYC =getDataList(instDate,SysParamUtil.getParam(SysParamConstants.CHANNEL_CODE_YC));
//		List<TblSettleDtl> dataListCJ =getDataList(instDate,SysParamUtil.getParam(SysParamConstants.TXN_CODE_CJ));
	...
	public String makeFile(String instDate, String channelId, String aimChnlId)
		...
		List<TblSettleDtl> dataList =getDataList(instDate,channelId);
	 ...
	 * 中信划付文件生成
	 */
	public String ZXfile(String instDate,List<TblSettleDtl> dataList) throws Exception {
		// TODO Auto-generated method stub
        ...
        if(payFile.createNewFile()){
        	...
        	try {
	        	...
	        	fw.write("-----------------------------------\n");
	        	for (TblSettleDtl tblPaySettleDtl : dataList) {
	        		...
	        		.append("|")											//6.空
	        		.append(SelectMethod.getPayTypeZXNew(null)
	        				.get(tblPaySettleDtl.getPayType()))
	        				.append("|")									//7.大小额标识
	        		.append(formatMoney(tblPaySettleDtl
	        				.getAmtSettle())).append("|")					//8.清算净额
	 ...
	 */
	@SuppressWarnings("unchecked")
	public String ZXWYfile(String instDate, List<TblSettleDtl> dataList) throws Exception {
		// TODO Auto-generated method stub
	 ...
	 * 工行划付文件生成
	 */
	public String GHfile(String instDate, List<TblSettleDtl> dataList) throws Exception {
		// TODO Auto-generated method stub
		String path=SysParamUtil.getParam(SysParamConstants.PAY_FILE_DISK)
				+CommonFunction.getCurrentDate().substring(0, 6)+"/";
		File payFile = new File(path, 
	 ...
	 * 邮储-银联划付文件生成
	 */
	public String YCfile(String instDate,List<TblSettleDtl> dataList,String channelId) throws Exception {
		// TODO Auto-generated method stub
		 TblPayChannelInfo tblPayChannelInfo =tblPayChannelInfoDAO
        ...
        if(payFile.createNewFile()){
        	...
        	try {
	        	...
	        	for (TblSettleDtl tblPaySettleDtl : dataList) {
	        		...
	        		.append(tblPaySettleDtl.getTblSettleDtlPK().getMchtId().length()==15?
	        				tblPaySettleDtl.getTblSettleDtlPK()
	        				.getDateStlm().substring(4)+tblPaySettleDtl
	        				.getTblSettleDtlPK().getMchtId()
	        				.substring(3)
	        			:tblPaySettleDtl.getTblSettleDtlPK()
	        				.getDateStlm().substring(4)+
	        				CommonFunction.fillString(tblPaySettleDtl
	    	        		.getTblSettleDtlPK().getMchtId().trim(), '0', 12, true))
	    	        		.append("|")									//2.订单号
	        		.append("156").append("|")								//3.交易币种
	        		.append(formatMoney(tblPaySettleDtl.getAmtSettle())
	        				.replace(".", "")).append("|")					//4.金额
	        		.append("|")											//5.产品类型
...
//				+ " and m.mcht_no=a.mcht_no "
		...
		factErrorAmt.append(" nvl(a.AMT_SETTLE,0)-"+backAmt);
...
//				+ "a.o_f13,"
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<TblSettleDtl> getDataList(String instDate,String channelId) throws Exception {
		// TODO Auto-generated method stub
		String hql=" from com.allinfinance.po.settleNew.TblSettleDtl t "
				+ "where  t.tblSettleDtlPK.dateStlm='"+instDate+"' "
				+ "and t.tblSettleDtlPK.channelId='"+channelId+"' "
				+ " and t.payType!='0' "
				+ "order by t.amtSettle desc ";
		List<TblSettleDtl> dataList =commQueryDAO.findByHQLQuery(hql);
		return dataList;
	}
	 ...
	 * @throws Exception
	 */
	private String[] payFormat(TblSettleDtl tblPaySettleDtl,List<Object[]> payAcctList,List<Object[]> cnapsIdList){
		...
		if(payAcctList!=null&&payAcctList.size()>0){
			for (Object[] payAcct : payAcctList) {
				if(tblPaySettleDtl.getMchtAcctNo().equals(payAcct[0].toString())
					...
					foramtRes[0]="2".equals(tblPaySettleDtl.getPayType())?"小额支付":"大额支付";
					foramtRes[1]="";
					foramtRes[2]="";
					return foramtRes;
				}
		...
		if(cnapsIdList!=null&&cnapsIdList.size()>0){
			for (Object[] cnapsId : cnapsIdList) {
				if(tblPaySettleDtl.getCnapsId().substring(0,3).equals(cnapsId[0].toString())){
					foramtRes[0]="2".equals(tblPaySettleDtl.getPayType())?"网银跨行支付":"大额支付";
	...
	private String  getSumAmt(String instDate,String channelId) throws Exception {
		// TODO Auto-generated method stub
		String sql=" select  to_char(nvl(sum(to_number(amt_settle)),0)) as amt_sum from TBL_SETTLE_DTL "
				+ "where date_stlm='"+instDate+"' and CHANNEL_ID='"+channelId+"' and pay_type!='0' ";
