...
public class T10215BOTarget implements T10215BO {
...
//		List<TblPaySettleDtl> dataListCJ =getDataList(settleDate,SysParamUtil.getParam(SysParamConstants.TXN_CODE_CJ));
		List<Object[]> dataListCJ =commQueryDAO.findBySQLQuery(getPayFileSqlCJ(settleDate, SysParamUtil.getParam(SysParamConstants.TXN_CODE_CJ),dateList));
...
//		List<String> sumDataSqlList=new ArrayList<String>();
		...
		String sqlDate3=payDtlCJ(settleDate,dateList);
...
//				+ " and d.CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
	...
	private String payDtlCJ(String settleDate,List<String> dateList){
...
//				+ " and d.CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
			...
			+ "),0)");
		errorSql.append("+nvl(("
				+ "select sum(nvl(t.AMT_SETTLMT,0)) from TBL_PAY_SETTLE_DTL t "
				+ " where t.mcht_no in (select mcht_no from TBL_PAY_SETTLE_MCHT_ERROR ) "
				+ " and t.inst_date='"+settleDate+"' "
				+ " and t.CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
				+ " and t.mcht_no=a.mcht_no "
			+ "),0)");
		errorSql.append("-nvl((select e.AMT from TBL_PAY_SETTLE_MCHT_ERROR e where e.mcht_no=a.mcht_no ),0)  ");
		
		String error="abs(case when "+errorSql+">0 then 0 else "+errorSql+" end  ) ";
		
		
		StringBuffer backAmt1 = new StringBuffer(); 
		backAmt1.append("nvl(("
				+ "select sum(nvl(m.samt,0)-nvl(m.amt1,0)) from TBL_ZTH m "
				+ " where m.rspcode='00' "
				+ " and m.eflag!='1' "
...
//				+ " and m.mcht_no=a.mcht_no "
		...
		backAmt2.append("nvl(("
				+ "select sum(nvl(k.amt,0)) from TBL_ZTH_DTL k "
				+ " where 1=1 "
...
//				+ " and m.mcht_no=a.mcht_no "
		...
		factErrorAmt.append(" nvl(a.AMT_SETTLMT,0)-"+error+"-"+backAmt);
	...
	private String repeatMcht(List<String> dateList){
		...
		amtSql.append("nvl(("
				+ "select sum(nvl(AMT_SETTLMT,0)) from TBL_PAY_SETTLE_DTL "
				+ " where mcht_no in (select mcht_no from TBL_PAY_SETTLE_MCHT_ERROR ) "
				+ " and inst_date='"+dateList.get(dateList.size()-1)+"' "
//				+ " and CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
				+ " and mcht_no=a.mcht_no "
			+ "),0) as V_1, ");
...
//				+ " and d.CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
		...
		StringBuffer errorSqlEnd = new StringBuffer();
		errorSqlEnd.append("nvl(("
				+ "select sum(nvl(d.AMT_SETTLMT,0)) from TBL_PAY_SETTLE_DTL d "
				+ " where d.mcht_no in (select mcht_no from TBL_PAY_SETTLE_MCHT_ERROR ) "
				+ " and d.inst_date<='"+dateList.get(dateList.size()-1)+"' "
				+ " and d.inst_date>='"+dateList.get(0)+"' "
//				+ " and d.CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
				+ " and d.mcht_no=a.mcht_no "
			+ "),0)");
		errorSqlEnd.append("-nvl(a.AMT,0) as errorend ");
...
//		excelReportCreator.setTitleList(titleList);
	 ...
	 * @throws Exception
	 */
	private String getPayFileSqlCJ(String instDate,String channelId,List<String> dateList) throws Exception {
		// TODO Auto-generated method stub
		StringBuffer errorSql = new StringBuffer(); 
		errorSql.append("nvl(("
				+ "select sum(nvl(d.AMT_SETTLMT,0)) from TBL_PAY_SETTLE_DTL d "
				+ " where d.mcht_no in (select mcht_no from TBL_PAY_SETTLE_MCHT_ERROR ) "
				+ " and d.inst_date<'"+instDate+"' "
				+ " and d.inst_date>='"+dateList.get(0)+"' "
//				+ " and d.CHANNEL_ID in(select CHANNEL_ID from TBL_PAY_CHANNEL_INFO where CHANNEL_FLAG ='1') "
				+ " and d.mcht_no=a.mcht_no "
			+ "),0)");
